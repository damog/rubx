class Twitter::Client
  ...
  # * +:post+
  # * +:delete+
  def status(action, value = nil, in_reply_to_status_id = nil)
    ...
    when :post
    	response = http_connect({:status => value, :source => @@config.source, :in_reply_to_status_id => in_reply_to_status_id }.to_http_str) {|conn| create_http_post_request(uri) }
    when :delete
